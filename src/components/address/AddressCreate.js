                            ...
                            <div className="card-body d-flex flex-column justify-content-between">
                                <div className="row">
                                    <div className="col-6">
                                        ...
                                        <p className="card-text"><strong>Thể loại:</strong> {bookDetail?.category?.name}</p>
                                        <p className="card-text text-danger"
                                           style={{
                                            ...
                                            {bookDetail && formatPrice(bookDetail.price)}
                                        </p>
                                    </div>
                                    {role === 'ROLE_USER' && (
                                        <div className="col-6">
                                            <div className="d-flex align-items-center my-3">
                                                ...
                                                <button className="btn btn-outline-secondary me-2" onClick={decrease}>-</button>
                                                <span className="me-2">{count}</span>
                                                <button className="btn btn-outline-secondary" onClick={increase}>+</button>
                                            </div>
                                            <div className="d-flex">
                                                <button className="btn btn-primary me-2" onClick={handleAddToCart}>Thêm vào giỏ hàng</button>
                                ...
                                <div>
                                    <div className="mt-3">
                                        ...
                                        <p>Gọi đặt hàng ngay: <span className="text-warning">0963258258</span> hoặc <span className="text-warning">0933109109</span></p>
                                        <h6>Thông tin & khuyến mãi</h6>
                                        <p>Đổi trả hàng trong vòng 7 ngày</p>
                                        <p>Freeship toàn quốc từ 250.000đ</p>
                                        {role === 'ROLE_SELLER' && bookDetail?.ownerId === accountId && (
                                            <>
                                                <Link to={`/editBook/${id}`} className="btn btn-primary">Chỉnh sửa</Link>
                                                <button onClick={() => handleShowModalDelete(bookDetail.id)} className="btn btn-danger m-lg-2">Xóa</button>
                                            </>
                                        )}
                                        <BookDelete show={show} setShow={setShow} onDeleteHandler={handleRemoveBook} />
                                    </div>
                                </div>
                            </div>
                        </div>
